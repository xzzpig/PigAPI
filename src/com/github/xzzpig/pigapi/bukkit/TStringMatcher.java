	...
	public static String buildStr(String str, LivingEntity entity, boolean isInt) {
		...
		if (entity instanceof Player) {
			...
			re = re.replaceAll("</fly/>", player.getAllowFlight() + "")
					...
					.replaceAll("</prefix/>",
							TPrefix.getPrefix(player.getName()))
					.replaceAll("</id/>", player.getName())
					.replaceAll("</colorid/>", colorid);
			if (re.contains("</prefix_")) {
				for (Entry<String, String> element : TPrefix.getPrefixMap(
						player.getName()).entrySet()) {
					re = re.replaceAll("</prefix_" + element.getKey() + "/>",
							element.getValue());
				}
			}
		}
